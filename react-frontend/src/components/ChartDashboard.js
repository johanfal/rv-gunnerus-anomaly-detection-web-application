...
export class ChartDashboard extends React.Component {
  ...
  componentDidUpdate(_prev_props, prev_state) {
    ...
    const deleted = prev_state.selected.filter(
  ...
  add_new_charts = (newly_selected, selected_items) => {
    for (let sig of newly_selected) {
      selected_items[sig] = this.add_chart(sig, sig, {
        id: undefined,
        time: undefined,
        signal: undefined,
      });
  ...
  store_reading = (values) => {
    ...
    for (let sig of selected) {
      chart_items[sig] = this.add_chart(sig, sig, {
        id: id,
        time: time,
        signal: values[sig],
      });
  ...
  render() {
    ...
    return (
      <div className="selector-chart-container">
        ...
        <div className="charts-container">{Object.values(chart_items)}</div>
      </div>
    );
  }
}
